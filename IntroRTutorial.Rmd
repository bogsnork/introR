...
```{r echo=TRUE}
veg.height <- c(44.2,22.7,NA,33.3)
veg.height
```
...
###
`"birds.csv"`  The name of the file you are importing.  Note that it is in quotes `""`: this is because we want **R** to read it as a string of characters.  
...
###
```{r, echo=TRUE, eval=FALSE}
birds <- read.table("birds.txt", sep="")
```
...
To complete this exercise you must first download the sample data `birds.csv` from [....] and save it to your C drive in a folder called "IntroR".  
...
**THIS DOESN'T WORK: CAN'T SET WORKING DIRECTORY**
...
### Excercise: birds
We will be using the dataframe birds imported earlier.  Have a look at it by typing View(birds).  
```{r block10-setup}
...
###
Lets look at the birds data again: 
```{r echo=TRUE}
birds
```
...
###
Lets look at the variables `species` and `farm`.  
```{r echo=TRUE}
plot(birds$farm ~ birds$species)
```

**R** has automatically chosen this graph type because we are plotting a numeric variable (farm) against a factor (species).  
...
```{r echo=TRUE}
boxplot(birds[,2:4], 
        main = "Number of birds (all species) by site",
        xlab = "Sites",
        ylab = "Bird count")
```
